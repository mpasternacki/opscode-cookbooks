...
server_auth_method - authentication with the server can be done with openid (apache2-only; using apache2::mod_auth_openid), htauth (basic), or none at all. The default is openid, "none" means no auth, any other value will use htauth (basic).
